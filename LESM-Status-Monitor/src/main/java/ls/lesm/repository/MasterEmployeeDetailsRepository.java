...
//public List<Object[]> getD(Integer id);
	 ...
	 @Query(nativeQuery=true,
	    ...
	    @Query(nativeQuery=true, value="SELECT master_emp_details.emp_id AS empId, master_emp_details.first_name AS firstName, master_emp_details.last_name AS lastName, master_emp_details.employee_id AS lancesoftId FROM master_emp_details WHERE master_emp_details.desg_fk= ?1")
        public List<SupervisorDropDown> supDropDown(@Param("id")int id);
	    
	    @Query(nativeQuery=true, value="SELECT master_emp_details.emp_id AS empId, master_emp_details.first_name AS firstName, master_emp_details.last_name AS lastName, master_emp_details.employee_id AS lancesoftId, designations.desg_id AS desgId  FROM ((master_emp_details LEFT JOIN departments ON master_emp_details.department_fk=departments.depart_id) LEFT JOIN designations ON master_emp_details.desg_fk=designations.desg_id) WHERE departments.depart='HR' AND designations.desg_names='General Manager' ")
	    public List<SupervisorDropDown> hrId();
	    
	    @Query(nativeQuery=true, value="SELECT master_emp_details.employee_id As lancesoftId, master_emp_details.first_name AS firstName, master_emp_details.last_name AS lastName FROM master_emp_details where master_emp_details.supervisor_fk= ?1 ")
